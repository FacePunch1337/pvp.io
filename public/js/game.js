...
export function startGame() {
  ...
  function preload() {
    ...
    this.load.spritesheet("playerIdle", "assets/player/idle.png", {
      frameWidth: 40,
      frameHeight: 40,
    });
    this.load.spritesheet("playerMove", "assets/player/run.png", {
      frameWidth: 40,
      frameHeight: 40,
    });
    this.load.spritesheet("playerFell", "assets/player/player_fell.png", {
      frameWidth: 40,
      frameHeight: 40,
    });
    this.load.spritesheet("pistolSprite", "assets/weapone/pistol/pistol.png", {
      frameWidth: 64,
      frameHeight: 32,
    });
    this.load.spritesheet(
      "pistolIdle",
      "assets/weapone/pistol/pistolidle.png",
    ...
    this.load.audio("pistolTakeSound", [
      "assets/sounds/takePistol.mp3",
      "assets/sounds/takePistol.ogg",
    ]);
    this.load.audio("pistolShootSound", [
      "assets/sounds/pistolShoot.mp3",
      "assets/sounds/pistolShoot.ogg",
    ]);
    this.load.audio("emptySound", [
      "assets/sounds/empty.mp3",
      "assets/sounds/empty.ogg",
    ]);
    this.load.audio("worldEmbiend", [
      "assets/sounds/world.mp3",
      "assets/sounds/world.ogg",
    ]);
  }
...
*/
    ...
    this.input.on(
      ...
      function (pointer) {
        if (this.input.mouse.locked) {
          ...
          if (distX > 800) {
            this.customCursor.x = player.x + 800;
          } else if (distX < -800) {
            this.customCursor.x = player.x - 800;
          }

          if (distY > 600) {
            this.customCursor.y = player.y + 600;
          } else if (distY < -600) {
            this.customCursor.y = player.y - 600;
          }
        }
    ...
    this.fellAnim = this.anims.create({
      key: "fell",
      frames: this.anims.generateFrameNumbers("playerFell", {
        start: 0,
        end: 3,
      }),
      frameRate: 10,
    ...
    this.pistolIdleAnim = this.anims.create({
      key: "pistol_idle",
      frames: this.anims.generateFrameNumbers("pistolIdle", {
        start: 0,
        end: 0,
      }),
      frameRate: 10,
    ...
    this.pistolShootAnim = this.anims.create({
      key: "pistol_shoot",
      frames: this.anims.generateFrameNumbers("pistolSprite", {
        start: 0,
        end: 2,
      }),
      frameRate: 10,
    ...
    this.socket.on("weaponPickedUp", function (weaponId, playerId, weaponType) {
      ...
      const pickedUpWeapon = self.weaponsGroup
        .getChildren()
    ...
    this.socket.on("weaponDrop", function (weaponId, playerId) {
      ...
      pistolTakeSound.play();
      const droppedWeapon = self.weaponsGroup
        .getChildren()
        .find((weapon) => weapon.id === weaponId);
      /*if(isPistol){
      playerWeapon = playerWeapons.find((weapon) => weapon.id === weaponId);
        ...
        self.weaponsGroup.getChildren().forEach(function (playerWeapon) {
          if (weaponData.weaponId === playerWeapon.id) {
            ...
            if (otherPlayer.playerId !== self.socket.id) {
              ...
              playerWeapon.setFlipX(
                calculateFlipX(weaponData.chel, weaponData.cursor)
              );
              ...
              if (weaponData.isPistol) {
                ...
                if (
                  weaponData.animationKey === "pistol_shoot" &&
                  weaponData.bulletsLength > 0
                ) {
                  playerWeapon.play("pistol_shoot", true);
                } else if (
                  weaponData.isShooting &&
                  weaponData.bulletsLength === 0
                ) {
                  emptySound.play();
                } else {
      ...
      self.otherPlayers.getChildren().forEach(function (otherPlayer) {
        self.weaponsGroup.getChildren().forEach(function (playerWeapon) {
          if (bulletData.weaponId === playerWeapon.id) {
            ...
            createBullet(
              bulletData.playerWeaponX,
              bulletData.playerWeaponY,
              bulletData.angle
            );
  ...
  function update() {
    ...
    const newCamX =
      camX + (targetX - game.config.width / 2 - camX) * cameraSpeed;
    const newCamY =
      camY + (targetY - game.config.height / 2 - camY) * cameraSpeed;
    ...
    if (playerWeapon && !isFell) {
      ...
      this.input.on("pointerdown", function (pointer) {
        if (
          ...
          canShoot &&
          playerWeapons.length > 0 &&
          bullets.filter((bullet) => bullet.active).length < poolLength
        ) {
          if (isPistol) {
  ...
  function spawnPlayer() {
    ...
    player = self.physics.add
      .sprite(randPosX, randPosY, "playerIdle")
      .setSize(25, 25);
  ...
  function constrainReticle(customCursor, radius) {
    ...
    // Ensures reticle cannot be moved offscreen
    if (distX > 800) {
      customCursor.x = player.x + 800;
    } else if (distX < -800) {
      customCursor.x = player.x - 800;
    }

    if (distY > 600) {
      customCursor.y = player.y + 600;
    } else if (distY < -600) {
      customCursor.y = player.y - 600;
    }
  ...
  function throwWeapon(player, weapon, targetX, targetY) {
    if (hasWeapon) {
      ...
      weapon.setVelocity(
        throwSpeed * Math.cos(angle),
        throwSpeed * Math.sin(angle)
      );
  ...
  function addOtherPlayers(self, playerInfo) {
    ...
    const existingPlayer = self.otherPlayers
      .getChildren()
      .find((player) => player.playerId === playerInfo.playerId);
    if (!existingPlayer) {
      const otherPlayer = self.add.sprite(
        playerInfo.x,
        playerInfo.y,
        "playerIdle"
      );
